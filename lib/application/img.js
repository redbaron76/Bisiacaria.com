...
Bisia.Img = {			// global Bisia in /lib/application/bisia.js
	 ...
	 */
	setAttachedImage: function(thumb, original) {
		...
		var input = $('<input/>', {type: 'hidden', name: 'image', value: thumb, id: 'image-url'});
		this.$imgButton.parent('form').append(input);

		var original = $('<input/>', {type: 'hidden', name: 'original', value: original, id: 'original-url'});
		this.$imgButton.parent('form').append(original);
	},
	 ...
	 * @return {Void}
	 */
	removeAttachedImage: function(version, fileId) {
		if (this.$imgButton) {
			...
			if (imageUrl.length > 0) {
				...
				Meteor.call('deleteServerImage', version, fileId, function(error, result) {
					if (error) {
						console.log(error);
					}
				});
