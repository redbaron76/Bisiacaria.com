...
Bisia.Delete = {
	...
	event: function(e, obj) {
		...
		if (obj.imageUrl.length > 0 && !obj.originalUrl) {
			Meteor.call('cloudinary_delete', Bisia.Img.getImagePublicId(obj.imageUrl), function(error, result) {
				if (error) {
					console.log(error);
				}
			});
		} else if (obj.originalUrl) {
			var fileName = Bisia.Upload.getFileNameFromFile(obj.originalUrl);
...
Meteor.methods({
	...
	deleteEvent: function(obj) {
		if (obj._id && obj.authorId == this.userId) {
			...
			Homepage.update({}, { $pull: { 'todayEvents': obj._id, 'tomorrowEvents': obj._id } });
			Events.remove(obj._id);
			return true;
		}
		return false
	},
	deletePost: function(obj) {
		if (obj._id && obj.authorId == this.userId) {
			...
			if (obj.position) {
				Users.update({ _id: this.userId },{'$unset': {'profile.position': '', 'loc': ''}});
			}
			return true;
		}
		return false;
