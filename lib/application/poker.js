...
Bisia.Poker = {
	...
	buildNewDeck: function(deckArr, instance) {
		if (deckArr.length == 5) {
			_.each(deckArr, function(value, i) {
				...
				// hide and show card after render
				$('.card[data-control=card' + i + ']').hide().show();
			});
			Meteor.setTimeout(function() {
				$('.card').addClass('open');
			}, 100);
	...
	buildChangeDeck: function(deckArr, instance) {
		if (deckArr.length == 5) {
			...
			if (!instance || instance.data.statusPlay != 'cambia') {
				Meteor.setTimeout(function() {
	...
	/**
	 * Called by cronjob rechargePokerCredits
	 * @return {Void}
	 */
	rechargeCredits: function() {
		Pokerplayers.update({}, {
			'$set': { 'credit': this.credit }
		}, {
			'multi': true
		});
	},
