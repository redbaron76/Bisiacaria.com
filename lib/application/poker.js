...
Bisia.Poker = {
	...
	generateNewDeck: function() {
		var deck = [];
		var avoid = arguments[0] || null;
		while(deck.length < 5){
			var rndNumber = _.random(1, 53);
			...
			for (var i = 0; i < deck.length; i++) {
				...
				if (avoid && _.contains(avoid, rndNumber)) {
					alreadyPresent = true;
					break;
				}
			}
	...
	generateChangeDeck: function(firstHand, cardsToKeep) {
		...
		if (_.isEmpty(cardsToKeep)) {
			...
			var avoidArray = firstHand.concat(cardsToKeep);
			var newDeck = this.generateNewDeck(avoidArray);
