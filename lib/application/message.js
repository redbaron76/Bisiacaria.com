...
Bisia.Message = {
	 ...
	 * @return {Bool}
	 */
	chatIsBanned: function() {
		return Chatusers.find({ 'userId': Meteor.userId(), 'isBanned': true }).count() > 0;
	},

	chatBanThisUser: function(e, userObj) {
		Meteor.call('chatRoomBanProposal', userObj.userId, function(error, success) {
			if (success) {
				Bisia.Ui.loadingRemove()
						.waitStop()
						.submitSuccess('La tua decisione è stata registrata correttamente.', 'Ban richiesto!', null, true);
			}
		});
	},
	...
	sendChatMessage: function(text) {
		...
		if (text && text.length > 0) {
			...
			Meteor.call('sendChatMessage', msgObj, function(error, success) {
				if(error)
					Bisia.log(error);
			});
		}
