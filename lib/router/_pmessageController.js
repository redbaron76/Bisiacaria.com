...
ChatRoomController = PaginatorBaseController.extend({
	increment: 100,
	findQuery: function() {
		return {};
	},
	subscriptions: function() {
		if (!!Meteor.userId()) {
			this.pageReady = Meteor.subscribe('chatRoom', this.findQuery(), this.findOptions());
		}
	},
	items: function() {
		return Chatroom.find(this.findQuery(), this.findOptions()).fetch().reverse();
	},
	nextPath: function() {
		return Router.routes.chatRoom.path({ 'pageLimit': this.pageLimit() + this.increment });
	},
	data: function() {
		var hasMore = this.items().length === this.pageLimit();
		return {
			items: this.items(),
			title: 'Bisia-Chat (' + Counts.get('countChatUsers') + ')',
			nextPath: hasMore ? this.nextPath() : null,
			pageReady: this.pageReady.ready()
		}
	}
