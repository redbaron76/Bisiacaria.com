...
BisPokerController = LoggedController.extend({
	...
	preloadImages: function() {
		var images = [];
		images.push('img/poker/back.jpg');
		for (var i = 0; i < 54; i++) {
			images.push('/img/poker/'+ i +'.jpg');
		}
		//this.state.set('preload', false);
		this.preload(images, 0);
	},
	preload: function(imageArray, index) {
		...
		if (imageArray && imageArray.length > index) {
			var img = new Image();
			img.onload = function () {
				parent.preload(imageArray, index + 1);
			}
			img.src = imageArray[index];

			if (index == 54) {
				parent.state.set('preloadPokerImages', true);
				console.log(index);
			}
		}
	},
	subscriptions: function() {
		...
		var pokerSub = Meteor.subscribe('pokerCurrentStatus');

		if (!this.state.get('preloadPokerImages') && !pokerSub.ready()) {
			this.preloadImages();
		}

		var sub = {};
		var imagePreload = this.state.get('preloadPokerImages');
		console.log(pokerSub, imagePreload);
		sub.ready = function() {
			return pokerSub.ready() && imagePreload;
		};
		return sub;
	},
	data: function() {
